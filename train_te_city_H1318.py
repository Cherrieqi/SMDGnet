...
H13_SE_img = np.load("data/City_H1318/gen_H13/SE_img.npy")
H13_SE_img = torch.from_numpy(H13_SE_img).float()
H13_img = np.load("data/City_H1318/gen_H13/img.npy")
H13_img = torch.from_numpy(H13_img).float()
H13_label = np.load("data/City_H1318/gen_H13/gt.npy")
H13_label = torch.LongTensor(H13_label)

H18_SE_img = np.load("data/City_H1318/gen_H18/SE_img.npy")
H18_SE_img = torch.from_numpy(H18_SE_img).float()
H18_img = np.load("data/City_H1318/gen_H18/img.npy")
H18_img = torch.from_numpy(H18_img).float()
H18_label = np.load("data/City_H1318/gen_H18/gt.npy")
H18_label = torch.LongTensor(H18_label)

AG_SE_img = np.load("data/City_H1318/gen_AG/SE_img.npy")
AG_SE_img = torch.from_numpy(AG_SE_img).float()
AG_img = np.load("data/City_H1318/gen_AG/img.npy")
AG_img = torch.from_numpy(AG_img).float()
AG_label = np.load("data/City_H1318/gen_AG/gt.npy")
AG_label = torch.LongTensor(AG_label)

TZ_SE_img = np.load("data/City_H1318/gen_TZ/SE_img.npy")
TZ_SE_img = torch.from_numpy(TZ_SE_img).float()
TZ_img = np.load("data/City_H1318/gen_TZ/img.npy")
TZ_img = torch.from_numpy(TZ_img).float()
TZ_label = np.load("data/City_H1318/gen_TZ/gt.npy")
TZ_label = torch.LongTensor(TZ_label)

CHI_SE_img = np.load("data/City_H1318/gen_CHI/SE_img.npy")
CHI_SE_img = torch.from_numpy(CHI_SE_img).float()
CHI_img = np.load("data/City_H1318/gen_CHI/img.npy")
CHI_img = torch.from_numpy(CHI_img).float()
CHI_label = np.load("data/City_H1318/gen_CHI/gt.npy")
CHI_label = torch.LongTensor(CHI_label)


SE_img = [H13_SE_img, H18_SE_img, AG_SE_img, TZ_SE_img, CHI_SE_img]
img = [H13_img, H18_img, AG_img, TZ_img, CHI_img]
label = [H13_label, H18_label, AG_label, TZ_label, CHI_label]

del H13_SE_img, H18_SE_img, AG_SE_img, TZ_SE_img, H13_img, H18_img, AG_img, TZ_img, H13_label, H18_label, AG_label, TZ_label, CHI_SE_img, CHI_img, CHI_label
gc.collect()
...
class_num = 4
domains_per_batch = 5
classes_per_domain = 4
samples_per_class = 51
num_batches_per_epoch = 122


work_dir = f'./work_dir/City_H1318_inch{in_ch}_w{w[0]}-{w[1]}-{w[2]}-{w[3]}_b{batch_size}_lr{learning_rate}/' \
