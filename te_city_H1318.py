...
from ImageDataset import ImgDataset_test
...
work_dir = f"./work_dir/City_H1318_inch9_w3-6-1-1_b1024_lr0.002/{seed}/"
for j in range(2):
    print(f"seed: {seed}")
    if j == 0:
        data_name = "PU"
    ...
    if data_name == "PU":
        SE_img = np.load("data/City_H1318/gen_PU/SE_img.npy")
        img = np.load("data/City_H1318/gen_PU/img.npy")
        label = np.load("data/City_H1318/gen_PU/gt.npy")
    else:
        SE_img = np.load("data/City_H1318/gen_PC/SE_img.npy")
        img = np.load("data/City_H1318/gen_PC/img.npy")
        label = np.load("data/City_H1318/gen_PC/gt.npy")
    ...
    with torch.no_grad():
        ...
        for i, data in loop:
            y, __, __, __, __, __ = model(data[0].to(device), data[1].to(device))
            gt = data[2][:, :class_num].argmax(dim=1).flatten().cpu().numpy()
            row_col = data[2][:, class_num:]
