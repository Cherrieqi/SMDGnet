...
#     :return:
#     """
#     label_slice_OH = np.zeros([len(label_slice), class_num])
#     for cls in range(class_num):
#         idx = np.where(label_slice == cls + 1)
#         if cls == 0:
#             label_slice_OH[idx] = [1, 0.2, 0.3, 0.1]
#         elif cls == 1:
#             label_slice_OH[idx] = [0.2, 1, 0.3, 0.1]
#         elif cls == 2:
#             label_slice_OH[idx] = [0.3, 0, 1, 0]
#         else:
#             label_slice_OH[idx] = [0.1, 0.3, 0.1, 1]

#     return torch.tensor(label_slice_OH)


def one_hot_slice_domain(label_slice, class_num, flag=1):
    """
    加上域标签
    :param label_slice: [N] torch.long
    :param class_num:
    :return:
    """
    label_slice_OH = torch.full([len(label_slice), class_num+1], 0.)
    for cls in range(class_num):
        idx = np.where(label_slice == cls + 1)
        label_slice_OH[idx, cls] = 1.

    label_slice_OH[:, -1] = flag
