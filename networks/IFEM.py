...
class conv_block(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=1):
        ...
        self.net_main = nn.Sequential(CBR(in_ch, out_ch, kernel_size=kernel_size, padding=0, groups=4),
                                      CBR(out_ch, 2 * out_ch, kernel_size=1, padding=0, groups=4),
                                      CBR(2 * out_ch, out_ch, kernel_size=1, padding=0))
        self.ca = ChannelAttention(out_ch)
        self.conv = CBR(in_ch, out_ch, kernel_size=kernel_size, padding=0, groups=4)
