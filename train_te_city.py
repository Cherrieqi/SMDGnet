...
PU_SE_img = np.load("data/City/gen_PU/SE_img.npy")
PU_SE_img = torch.from_numpy(PU_SE_img).float()
PU_img = np.load("data/City/gen_PU/img.npy")
PU_img = torch.from_numpy(PU_img).float()
PU_label = np.load("data/City/gen_PU/gt.npy")
PU_label = torch.LongTensor(PU_label)

PC_SE_img = np.load("data/City/gen_PC/SE_img.npy")
PC_SE_img = torch.from_numpy(PC_SE_img).float()
PC_img = np.load("data/City/gen_PC/img.npy")
PC_img = torch.from_numpy(PC_img).float()
PC_label = np.load("data/City/gen_PC/gt.npy")
PC_label = torch.LongTensor(PC_label)

AG_SE_img = np.load("data/City/gen_AG/SE_img.npy")
AG_SE_img = torch.from_numpy(AG_SE_img).float()
AG_img = np.load("data/City/gen_AG/img.npy")
AG_img = torch.from_numpy(AG_img).float()
AG_label = np.load("data/City/gen_AG/gt.npy")
AG_label = torch.LongTensor(AG_label)
#
TZ_SE_img = np.load("data/City/gen_TZ/SE_img.npy")
TZ_SE_img = torch.from_numpy(TZ_SE_img).float()
TZ_img = np.load("data/City/gen_TZ/img.npy")
TZ_img = torch.from_numpy(TZ_img).float()
TZ_label = np.load("data/City/gen_TZ/gt.npy")
TZ_label = torch.LongTensor(TZ_label)

CHI_SE_img = np.load("data/City/gen_CHI/SE_img.npy")
CHI_SE_img = torch.from_numpy(CHI_SE_img).float()
CHI_img = np.load("data/City/gen_CHI/img.npy")
CHI_img = torch.from_numpy(CHI_img).float()
CHI_label = np.load("data/City/gen_CHI/gt.npy")
CHI_label = torch.LongTensor(CHI_label)

SE_img = [PU_SE_img, PC_SE_img, AG_SE_img, TZ_SE_img, CHI_SE_img]
img = [PU_img, PC_img, AG_img, TZ_img, CHI_img]
label = [PU_label, PC_label, AG_label, TZ_label, CHI_label]

del PU_SE_img, PC_SE_img, AG_SE_img, TZ_SE_img, PU_img, PC_img, AG_img, TZ_img, PU_label, PC_label, AG_label, TZ_label, CHI_SE_img, CHI_img, CHI_label
gc.collect()
...
work_dir = f'./work_dir/City_inch{in_ch}_w{w[0]}-{w[1]}-{w[2]}-{w[3]}_b{batch_size}_lr{learning_rate}/' \
...
for epoch in range(0, num_epoch):
    ...
    for i, data in loop:
        ...
        optimizer, learning_rate = lr_adj(num_iters, change_num, learning_rate, optimizer, rate, lr_warmup=0.000001, len_warmup=0)
        optimizer.zero_grad()
