...
HC_SE_img = np.load("data/Farmland/gen_HC/SE_img.npy")
HC_SE_img = torch.from_numpy(HC_SE_img).float()
HC_img = np.load("data/Farmland/gen_HC/img.npy")
HC_img = torch.from_numpy(HC_img).float()
HC_label = np.load("data/Farmland/gen_HC/gt.npy")
HC_label = torch.LongTensor(HC_label)

IP_SE_img = np.load("data/Farmland/gen_IP/SE_img.npy")
IP_SE_img = torch.from_numpy(IP_SE_img).float()
IP_img = np.load("data/Farmland/gen_IP/img.npy")
IP_img = torch.from_numpy(IP_img).float()
IP_label = np.load("data/Farmland/gen_IP/gt.npy")
IP_label = torch.LongTensor(IP_label)

CHI_SE_img = np.load("data/Farmland/gen_CHI/SE_img.npy")
CHI_SE_img = torch.from_numpy(CHI_SE_img).float()
CHI_img = np.load("data/Farmland/gen_CHI/img.npy")
CHI_img = torch.from_numpy(CHI_img).float()
CHI_label = np.load("data/Farmland/gen_CHI/gt.npy")
CHI_label = torch.LongTensor(CHI_label)

SE_img = [HC_SE_img, IP_SE_img, CHI_SE_img]
img = [HC_img, IP_img, CHI_img]
label = [HC_label, IP_label, CHI_label]

del HC_SE_img, IP_SE_img, CHI_SE_img, HC_img, IP_img, CHI_img, HC_label, IP_label, CHI_label
gc.collect()
...
samples_per_class = 113
num_batches_per_epoch = 185

work_dir = f'./work_dir/Farmland_inch{in_ch}_w{w[0]}-{w[1]}-{w[2]}-{w[3]}_b{batch_size}_lr{learning_rate}/' \
...
for epoch in range(0, num_epoch):
    ...
    for i, data in loop:
        ...
        optimizer, learning_rate = lr_adj(num_iters, change_num, learning_rate, optimizer, rate,
                                          lr_warmup=0.000001,
                                          len_warmup=0)
        optimizer.zero_grad()
