...
rate_train_PU   = [4000, 5029,  3064,  4000]
rate_train_PC   = [3090, 6584,  4000,  4000]
...
PU_image, PU_gt = set_division_pro(4, [1, 2, 3, 4], PU_img_shuffle, PU_gt_shuffle, 'train', rate_train_PU)
PU_image = normHSI_smp_s(PU_image)
del PU_img_shuffle, PU_gt_shuffle
gc.collect()

PU_gt_OH = one_hot_slice_domain(PU_gt, class_num=class_num, flag=1)
del PU_gt
gc.collect()

np.save(f"data/City/gen_PU/gt.npy", PU_gt_OH)
del PU_gt_OH
gc.collect()

np.save(f"data/City/gen_PU/img.npy", PU_image*1.5-0.1)

PU_SE_image = data_trans(PU_image*1.5-0.1)
del PU_image
gc.collect()

np.save(f"data/City/gen_PU/SE_img.npy", PU_SE_image)
del PU_SE_image
gc.collect()
...
PC_image, PC_gt = set_division_pro(4, [1, 2, 3, 4], PC_img_shuffle, PC_gt_shuffle, 'train', rate_train_PC)
PC_image = normHSI_smp_s(PC_image)
del PC_img_shuffle, PC_gt_shuffle
gc.collect()

PC_gt_OH = one_hot_slice_domain(PC_gt, class_num=class_num, flag=2)
del PC_gt
gc.collect()

np.save(f"data/City/gen_PC/gt.npy", PC_gt_OH)
del PC_gt_OH
gc.collect()

np.save(f"data/City/gen_PC/img.npy", PC_image*1.5-0.1)

PC_SE_image = data_trans(PC_image*1.5-0.1)
del PC_image
gc.collect()

np.save(f"data/City/gen_PC/SE_img.npy", PC_SE_image)
del PC_SE_image
gc.collect()
...
np.save(f"data/City/gen_AG/gt.npy", AG_gt_OH)
del AG_gt_OH
gc.collect()

np.save(f"data/City/gen_AG/img.npy", AG_image)

AG_SE_image = data_trans(AG_image)
del AG_image
gc.collect()

np.save(f"data/City/gen_AG/SE_img.npy", AG_SE_image)
del AG_SE_image
gc.collect()
...
np.save(f"data/City/gen_TZ/gt.npy", TZ_gt_OH)
del TZ_gt_OH
gc.collect()

np.save(f"data/City/gen_TZ/img.npy", TZ_image)

TZ_SE_image = data_trans(TZ_image)
del TZ_image
gc.collect()

np.save(f"data/City/gen_TZ/SE_img.npy", TZ_SE_image)
del TZ_SE_image
gc.collect()
...
np.save(f"data/City/gen_CHI/gt.npy", CHI_gt_OH)
del CHI_gt_OH
gc.collect()

np.save(f"data/City/gen_CHI/img.npy", CHI_image)

CHI_SE_image = data_trans(CHI_image)
del CHI_image
gc.collect()

np.save(f"data/City/gen_CHI/SE_img.npy", CHI_SE_image)
del CHI_SE_image
gc.collect()
...
np.save(f"data/City/gen_H13/gt.npy", torch.cat((H13_gt_OH, H13_point_idx), dim=1))
del H13_gt_OH
gc.collect()

np.save(f"data/City/gen_H13/img.npy", H13_image*2.8-0.45)

H13_SE_image = data_trans(H13_image*2.8-0.45)
del H13_image
gc.collect()

np.save(f"data/City/gen_H13/SE_img.npy", H13_SE_image)
del H13_SE_image
gc.collect()
...
np.save(f"data/City/gen_H18/gt.npy", torch.cat((H18_gt_OH, H18_point_idx), dim=1))
del H18_gt_OH
gc.collect()

np.save(f"data/City/gen_H18/img.npy", H18_image*2.2-0.05)

H18_SE_image = data_trans(H18_image*2.2-0.05)
del H18_image
gc.collect()

np.save(f"data/City/gen_H18/SE_img.npy", H18_SE_image)
del H18_SE_image
gc.collect()
